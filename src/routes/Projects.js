		...
		<div className="PageContent">
			...
			<div className="divBlock">
				<div className="halfBlock">
					<p>The KHUX Info Page is my first major personal project. In 2016, I delved into the mobile game Kingdom Hearts Union χ and quickly became a community resource for information. However, as my tasks grew in complexity, I realized that they would not be effectively solved by 280-character social media posts.</p>
                    <p>This led to the creation of the KHUX Info Page, a website with multiple pages to disperse data to the community. The first challenge I solved was allowing users to effectively plan their equipment upgrades, which uses limited resources. I created a cost calculator page that showed users the material cost required and stat growth gained from a start level to an end level. Since the cost calculator's release, the site has expanded to include tools for searching, sorting, and filtering over the 2,000 characters that the game features. This search tool harnessed JavaScript and HTML to allow users to perform operations over the dataset and view outputted characters in concise info cards.</p>
                    <p>The website was a huge success, resulting in over 34,000 users and 77,000 visits, with users logging in from all over the world. It was a gratifying payoff and inspired me to continue developing and maintaining the web app for the community.</p>
				</div>
				<div className="halfBlock">
			...
			<div className="divBlock">
				...
				<div className="halfBlock">
					<p>My next major website is for the massively multiplayer online game Final Fantasy 14. I organized several multi-person coordinated events where groups of eight dedicated players participated in raids. In these raids, players must perform complex sequences of inputs, including executing an optimized rotation of actions and performing movements necessary to avoid boss attacks and losing the battle.</p>
                    ...
                    <p>I created page guides for each of the latest fights in the game. Each guide was populated with images, videos, and text created by me. Over 50 players in my circle of raiders benefited from my project. With this guide as their aid, many groups of players were able to successfully team up, clear challenges, and obtain achievements in the game that less than 10% of the players own.</p>
				</div>
			</div>
			<br />
			<br />
			...
			<div className="divBlock">
				<div className="halfBlock">
					<p>My first Python project was Data Sora. I was determined to solve a challenge in the Kingdom Hearts Union χ community. There were two issues. First, the in-game notice list was only available in the in-game browser, and many browser features were disabled. Second, the notice list only displayed the 20 most recent notices, which limited players' ability to view history. With my Data Sora project, I created a solution for both issues.</p>
                    <p>The purpose of Data Sora is to track notices, web scrape the contents of notices, and post formatted content to Discord, a popular social media service. I used context menu options on an Android device and some workarounds in order to find the URL of the notice page. I stored the notice URL, the notice name, and the date that the notice was posted into a JSON. This would allow me to track notice history for archival purposes.</p>
                    <p>Using the BeautifulSoup Python library, I scraped page contents given the URL. I further used the library to format the text for posting to Discord. Then I used Discord's API to send the text to specified server channels. I hosted the Python file on a file hosting service, which allowed it to automatically check for new notices every 30 minutes. New notices would be recorded, scraped, and posted to Discord. Thousands of users benefitted from being able to see new notices without loading and opening the game, and the log of Discord posts allowed users to scroll back through Data Sora posts to gain information about the past.</p>
				</div>
				<div className="halfBlock">
			...
			<div className="divBlock">
				...
				<div className="halfBlock">
					<p>I created a second Python app focused on the Final Fantasy 14 community. In the high-end raiding community, some users are willing to pay players in-game currency, called Gil, to assist them in battles. However, there are two roadblocks with fully capitalizizing on these opportunities. First, the in-game player request board (Party Finder) has no way to tag or filter to tell other players that the host is willing to pay in-game currency. Second, these party requests often fill up quickly, and there is no way to know if a currency listing has been opened outside of refreshing the Party Finder constantly, which is inefficient. The Gil Checker bot was created with the goal of giving myself and my teammates an edge over the money-making competition.</p>
                    ...
                    <p>The next step is to find the listings that were created by paying players. Since there was no filter or tag for money parties, I used word filtering and regex to find the desired listings. These requests often had text such as "paying everyone 4M to clear this fight" or "I will trade the loot in exchange for 500k." With regex, I only returned listings containing a numerical value followed by an abbreviation of quantity, such as M, Mil, or K (not case sensitive).</p>
					<p>Finally, I formatted the selected listings and posted them to Discord. The listings were formatted into blocks with relevant information, such as how many players and what roles were still needed. I also added the minimum power level required to join as well as the location to join from. The Python bot checks for relevant listings every minute and is hosted on a file hosting service. The end result was over 250 million Gil collected in a game where 5 million Gil is considered a large amount. It also provided a unique team building experience between me and my teammates as we raced to join parties whenever the Discord bot notified us of a new listing. Gil Checker was a fun project that put more of my Python skills to practice in an everyday use-case. There were immediate benefits for both myself and my teammates.</p>
				</div>
			</div>
			<br />
			<br />
			...
			<div className="divBlock">
				<div className="halfBlock">
					<p>The portfolio you see now is the result of many cycles of iteration. In the past, my portfolio solely ran on JavaScript and HTML. It was a raw project created in the early phases of my career. Since then, I have learned more languages and techniques to create a more sophisticated website.</p>
					<p>The current iteration utilizes React. I learned how to build and deploy a React application to GitHub Pages. I utilized GitHub Actions to create a workflow that automatically builds and deploys the app every time a change is pushed, which follows CI / CD practices. It was invigorating to harness the potential of technologies I had previously only scratched the surfaces of.</p>
				</div>
				<div className="halfBlock">
